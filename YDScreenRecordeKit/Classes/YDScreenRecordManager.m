...
@interface YDScreenRecordManager ()
...
- (YDVideoWriteServer *)writeServer {
    if (!_writeServer) {
        ...
        _writeServer = [[YDVideoWriteServer alloc] initWithFinishCompletion:^(NSString * _Nonnull path) {
            ...
            if (strongself.stopBlock) {
                AVAsset *asset = [AVAsset assetWithURL:[NSURL fileURLWithPath:path]];
                if (asset && asset.duration.value>0) {
                    strongself.stopBlock([path componentsSeparatedByString:@"/"].lastObject, asset.duration.value/asset.duration.timescale, strongself.startTime, path);
                }
            }
        }];
    }
